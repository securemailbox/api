...
def retrieve():
    ...
    if req_fingerprint is None:
        return (
            jsonify(
                {"success": False, "error": "fingerprint not in request", "data": None}
            ),
            400,
        )
    ...
    if not isinstance(req_fingerprint, str):
        return (
            jsonify(
                {"success": False, "error": "fingerprint value not valid", "data": None}
            ),
            400,
        )
    ...
    if mailbox_id is None:
        return (
            jsonify(
                {"success": False, "error": "fingerprint not in database", "data": None}
            ),
            400,
        )
    ...
    try:
        if req_sender_fingerprint:
            message_receive = Message.query.filter_by(
                mailbox_id=mailbox_id, sender_fingerprint=req_sender_fingerprint
            ).all()
        else:
        ...
        return (
            jsonify(
                {
                    "success": False,
                    "error": "Unknown error receiving messages",
                    "data": None,
                }
            ),
            400,
